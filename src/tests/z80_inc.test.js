...
describe('INC', function() {
  ...
  for(let i = 0; i < combinations8bit.length; i += 1) {
    ...
    let desc = `INC ${modeText(c.dest, c.mode)}`
    describe(desc, function() {
      makeGenericTest('increment resulting in zero', 'INC', c.dest, c.mode, c.dest, c.mode, c.offset, 0xFF, 0xFF, 0x00, c.opcodes, c.length, {Z: true, N: false}, ["PC", c.dest])
      makeGenericTest('increment resulting in overflow', 'INC', c.dest, c.mode, c.dest, c.mode, c.offset, 0x7F, 0x7F, 0x80, c.opcodes, c.length, {P: true, N: false}, ["PC", c.dest])
      makeGenericTest('increment resulting in sign', 'INC', c.dest, c.mode, c.dest, c.mode, c.offset, 0x7F, 0x7F, 0x80, c.opcodes, c.length, {S: true, N: false}, ["PC", c.dest])
      makeGenericTest('increment resulting in half carry', 'INC', c.dest, c.mode, c.dest, c.mode, c.offset, 0x7F, 0x7F, 0x80, c.opcodes, c.length, {H: true, N: false}, ["PC", c.dest])
    })
  }
  ...
  for(let i = 0; i < combinations16bit.length; i += 1) {
    ...
    describe(`INC ${c.dest}`, function() {
      makeGenericTest('increment resulting in zero', 'INC', c.dest, 'register16', c.dest, 'register16', 0, 0xFFFF, 0xFFFF, 0x0000, c.opcodes, c.length, {}, ["PC", c.dest])
      makeGenericTest('increment resulting in non zero', 'INC', c.dest, 'register16', c.dest, 'register16', 0, 0x0001, 0x0001, 0x0002, c.opcodes, c.length, {}, ["PC", c.dest])
    })
  }
